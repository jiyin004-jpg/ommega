...
use crate::keymaster::utils::{
    ...
    once_lock_get_or_try_init, AppUid,
};
...
use kmr_crypto_ommega::ec::OmmegaEc;
use kmr_crypto_ommega::hmac::OmmegaHmac;
use kmr_crypto_ommega::rng::OmmegaRng;
use kmr_crypto_ommega::rsa::OmmegaRsa;
...
impl KeyMintWrapper {
    ...
    pub fn localize_auth_token(&self, auth_token: &HardwareAuthToken) -> Result<HardwareAuthToken> {
        ...
        let mac = kmr_common::crypto::hmac_sha256(&OmmegaHmac, &hmac_key.0, &mac_input)
            .map_err(|error| anyhow::anyhow!("{error:?}"))
...
fn init_keymint_ta(security_level: SecurityLevel, config: &Config) -> Result<KeyMintTa> {
    ...
    let rsa = OmmegaRsa::default();
    let ec = OmmegaEc::default();
    let hkdf: Box<dyn kmr_common::crypto::Hkdf> = Box::new(OmmegaHmac);
    let imp = kmr_common::crypto::Implementation {
        rng: Box::new(rng),
        clock: Some(Box::new(clock)),
        compare: Box::new(kmr_crypto_ommega::eq::OmmegaEq),
        aes: Box::new(kmr_crypto_ommega::aes::OmmegaAes),
        des: Box::new(kmr_crypto_ommega::des::OmmegaDes),
        hmac: Box::new(OmmegaHmac),
        rsa: Box::new(rsa),
        ec: Box::new(ec),
        ckdf: Box::new(kmr_crypto_ommega::aes_cmac::OmmegaAesCmac),
        hkdf,
        sha256: Box::new(kmr_crypto_ommega::sha256::OmmegaSha256),
        mldsa: Box::new(kmr_crypto_ommega::mldsa::OmmegaMlDsa),
    };
    ...
    // Remote backend. Both the TEE and STRONGBOX security levels route
    // attestation the same way (three-end consistent): through the relay
    // (`RemoteRelayBackend`) when `[remote].enabled` is set, otherwise through
    // the local software keybox (`sign_info` above). The requesting security
    // level is forwarded to the relay (`attestation_security_level`) so the
    // minted chain is tagged TEE vs StrongBox correctly. STRONGBOX is not
    // routed through a dedicated backend — driving the real `/strongbox` HAL
    // from the keystore context failed on devices where that HAL is declared
    // but not servable, so it is not wired in.
    let remote: Option<Box<dyn kmr_ta::device::RemoteBackend>> = {
        if config.remote.enabled {
            log::info!("remote TEE relay backend enabled");
        }
        Some(Box::new(crate::remote::RemoteRelayBackend))
    };
...
pub fn localize_auth_token_for_ommega(auth_token: &HardwareAuthToken) -> Result<HardwareAuthToken> {
...
fn shared_keymint_wrapper_inner(security_level: SecurityLevel) -> Result<Arc<KeyMintWrapperInner>> {
    ...
    let snapshot = config()
        ...
        .clone();
    once_lock_get_or_try_init(wrapper, || {
...
mod tests {
    ...
    fn test_crypto() -> kmr_common::crypto::Implementation {
        kmr_common::crypto::Implementation {
            rng: Box::new(OmmegaRng),
            clock: Some(Box::new(clock::StdClock)),
            compare: Box::new(kmr_crypto_ommega::eq::OmmegaEq),
            aes: Box::new(kmr_crypto_ommega::aes::OmmegaAes),
            des: Box::new(kmr_crypto_ommega::des::OmmegaDes),
            hmac: Box::new(OmmegaHmac),
            rsa: Box::new(OmmegaRsa::default()),
            ec: Box::new(OmmegaEc::default()),
            ckdf: Box::new(kmr_crypto_ommega::aes_cmac::OmmegaAesCmac),
            hkdf: Box::new(OmmegaHmac),
            sha256: Box::new(kmr_crypto_ommega::sha256::OmmegaSha256),
            mldsa: Box::new(kmr_crypto_ommega::mldsa::OmmegaMlDsa),
        }
    }
