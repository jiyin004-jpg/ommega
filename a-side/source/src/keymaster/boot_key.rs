...
use crate::keymaster::utils::once_lock_get_or_try_init;
use anyhow::{Context, Result};
use kmr_common::crypto::AES_256_KEY_LENGTH;
use kmr_crypto_ommega::km::{hkdf_expand, ommega_legacy_kdf_expand};
use kmr_crypto_ommega::zvec::ZVec;
use log::{error, info};
...
fn get_level_zero_key_km_and_strategy() -> Result<(KeyMintDevice, DenyLaterStrategy)> {
    let &(level, strategy) = once_lock_get_or_try_init(&LEVEL_ZERO_SELECTION, || -> Result<_> {
...
impl BootLevelKeyCache {
    ...
    fn get_hkdf_key(&mut self, boot_level: BootLevel) -> Result<Option<&ZVec>> {
        ...
        // Bound how far the cache can be grown in one step. `keystore.boot_level`
        // is only advanced a handful of times during a normal boot, but the
        // property is writable via resetprop, so an adversarial value must not
        // be allowed to allocate gigabytes of HKDF keys while holding the lock.
        const MAX_CACHE_GROWTH: usize = 4096;
        if boot_level.0.saturating_sub(first_not_cached) > MAX_CACHE_GROWTH {
            error!(
                "boot level {boot_level:?} too far ahead of cache ({} entries); refusing to grow",
                MAX_CACHE_GROWTH
            );
            return Ok(None);
        }
...
/// Boot-level keys produced by the accidental KDF used by older ommega builds.
