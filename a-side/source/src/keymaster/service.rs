...
use crate::keymaster::permission::{
    check_grant_permission, check_key_permission, check_keystore_permission, require_ommega_ctx,
...
use crate::top::jiyin004::ommega::ISecurityLevel::ISecurityLevel;
...
struct SecurityLevels {
    ...
    i_osec_level_by_uuid: HashMap<Uuid, Strong<dyn ISecurityLevel>>,
...
impl KeystoreService {
    ...
    pub fn new_native_binder() -> Result<KeystoreService> {
        let result = Self {
            // StrongBox is only offered when the A-side natively exposes a
            // StrongBox KeyMint HAL; otherwise it behaves exactly like a device
            // without StrongBox (HARDWARE_TYPE_UNAVAILABLE). The STRONGBOX
            // security level is served by the same key source as TEE (local
            // keybox in local mode, the relay in remote mode) — it is NOT wired
            // to the real `/strongbox` HAL. The attestation extension is simply
            // tagged securityLevel=StrongBox.
    ...
    fn get_custom_security_level(
        &self,
        sec_level: SecurityLevel,
    ) -> Result<Strong<dyn ISecurityLevel>> {
        self.ensure_security_levels_current()?;
        let security_levels = self.security_levels.read().unwrap();
        if let Some(dev) = security_levels
            .uuid_by_sec_level
    ...
    fn is_grant(&self, ctx: Option<&CallerInfo>, grant: &KeyDescriptor) -> Result<bool> {
        ...
        match DB.with(|db| {
            ...
            Err(error) => match error.root_cause().downcast_ref::<Error>() {
                ...
                _ => Err(error).context(err!("KeystoreService::is_grant.")),
            },
        }
    }
...
#[allow(non_snake_case)]
impl IKeymintService for KeystoreService {
    fn getSecurityLevel(
        &self,
        security_level: SecurityLevel,
    ) -> Result<Strong<dyn IKeystoreSecurityLevel>, Status> {
        let _wp = wd::watch_millis_with("IKeymintService::getSecurityLevel", 500, security_level);
        self.get_security_level(security_level)
            .map_err(into_logged_binder)
    }

    fn getCustomSecurityLevel(
        &self,
        security_level: SecurityLevel,
    ) -> Result<Strong<dyn ISecurityLevel>, Status> {
        let _wp = wd::watch_millis_with("IKeymintService::getSecurityLevel", 500, security_level);
        self.get_custom_security_level(security_level)
            .map_err(into_logged_binder)
    }
    ...
    fn getKeyEntry(
        ...
        let _wp = wd::watch("IKeymintService::get_key_entry");
        let ctx = Some(require_ommega_ctx(ctx, "IKeymintService::getKeyEntry")?);
    ...
    fn listEntries(
        ...
        let _wp = wd::watch("IKeymintService::listEntries");
        let ctx = Some(require_ommega_ctx(ctx, "IKeymintService::listEntries")?);
        self.list_entries(ctx, domain, namespace)
            .map_err(into_logged_binder)
    }

    fn deleteKey(&self, ctx: Option<&CallerInfo>, key: &KeyDescriptor) -> Result<(), Status> {
        let _wp = wd::watch("IKeymintService::deleteKey");
        let ctx = Some(require_ommega_ctx(ctx, "IKeymintService::deleteKey")?);
    ...
    fn grant(
        ...
        let _wp = wd::watch("IKeymintService::grant");
        let ctx = Some(require_ommega_ctx(ctx, "IKeymintService::grant")?);
    ...
    fn ungrant(
        ...
        let _wp = wd::watch("IKeymintService::ungrant");
        let ctx = Some(require_ommega_ctx(ctx, "IKeymintService::ungrant")?);
    ...
    fn getSupplementaryAttestationInfo(&self, tag: Tag) -> Result<Vec<u8>, Status> {
        let _wp = wd::watch("IKeymintService::getSupplementaryAttestationInfo");
        self.get_supplementary_attestation_info(tag)
            .map_err(into_logged_binder)
    }
    ...
    fn updateEcKeybox(
        ...
        let ctx = require_ommega_ctx(ctx, "IKeymintService::updateEcKeybox")?;
    ...
    fn updateRsaKeybox(
        ...
        let ctx = require_ommega_ctx(ctx, "IKeymintService::updateRsaKeybox")?;
        self.enforce_keybox_admin(ctx).map_err(into_logged_binder)?;
        let chain: Vec<kmr_wire::keymint::Certificate> = chain
            .iter()
            .map(|c| kmr_wire::keymint::Certificate {
    ...
    fn isGrant(&self, ctx: Option<&CallerInfo>, grant: &KeyDescriptor) -> Result<bool, Status> {
        let _wp = wd::watch("IKeymintService::isGrant");
        let ctx = Some(require_ommega_ctx(ctx, "IKeymintService::isGrant")?);
        self.is_grant(ctx, grant).map_err(into_logged_binder)
    }
}
