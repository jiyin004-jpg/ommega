...
use crate::keymaster::keymint_device::{localize_auth_token_for_ommega, KeyMintDevice};
...
use crate::top::jiyin004::ommega::IAuthorizationService::{
    BnAuthorizationService, IAuthorizationService,
};
use crate::{
    config::config,
...
impl AuthorizationManager {
    ...
    pub fn new_ommega_binder() -> Result<Strong<dyn IAuthorizationService>> {
        let mgr = Self::new_manager();
        Ok(BnAuthorizationService::new_binder_with_features(
            mgr,
            crate::consts::sid_features(),
        ))
    }
    ...
    fn add_auth_token(
        ...
        let localized = localize_auth_token_for_ommega(auth_token)
            .context(ks_err!("localizing trusted auth token for ommega"))?;
...
fn check_ommega_keystore_permission(
    ctx: &CallerInfo,
    perm: KeystorePerm,
    label: &str,
) -> BinderResult<()> {
...
impl IAuthorizationService for AuthorizationManager {
    fn addAuthToken(
        &self,
        ctx: Option<&CallerInfo>,
        auth_token: &HardwareAuthToken,
    ) -> BinderResult<()> {
        let _wp = wd::watch("IAuthorizationService::addAuthToken");
        let ctx = require_ommega_ctx(ctx, "IAuthorizationService::addAuthToken")?;
    ...
    fn onDeviceUnlocked(
        ...
        let _wp = wd::watch("IAuthorizationService::onDeviceUnlocked");
        let ctx = require_ommega_ctx(ctx, "IAuthorizationService::onDeviceUnlocked")?;
        check_ommega_keystore_permission(
            ctx,
            KeystorePerm::Unlock,
    ...
    fn getAuthTokensForCredStore(
        ...
        let _wp = wd::watch("IAuthorizationService::getAuthTokensForCredStore");
        let ctx = require_ommega_ctx(ctx, "IAuthorizationService::getAuthTokensForCredStore")?;
        check_ommega_keystore_permission(
            ctx,
    ...
    fn getLastAuthTime(
        ...
        let _wp = wd::watch("IAuthorizationService::getLastAuthTime");
        let ctx = require_ommega_ctx(ctx, "IAuthorizationService::getLastAuthTime")?;
        check_ommega_keystore_permission(
            ctx,
