...
use crate::keymaster::permission::{
    ...
    check_unique_id_attestation_permissions, require_ommega_ctx, KeyPerm,
};
...
use crate::top::jiyin004::ommega::{
    CallerInfo::CallerInfo,
    ISecurityLevel::{BnSecurityLevel, ISecurityLevel},
};
...
struct OmmegaSecurityLevelWrapper {
    inner: Arc<KeystoreSecurityLevel>,
}
...
impl Deref for OmmegaSecurityLevelWrapper {
    type Target = KeystoreSecurityLevel;
...
impl KeystoreSecurityLevel {
    ...
    pub fn new_binders(
        ...
        let ommega = BnSecurityLevel::new_binder_with_features(
            OmmegaSecurityLevelWrapper { inner },
            crate::consts::sid_features(),
        );
        Ok((aosp, ommega))
    }
    ...
    fn create_operation(
        ...
        let km_blob = SUPER_KEY
            .read()
            .unwrap()
            .unwrap_key_if_required_with_ommega_compatibility(&blob_metadata, km_blob)
            .context(ks_err!("Failed to handle super encryption."))?;
    ...
    fn generate_key(
        ...
        // A user-generated attestation key signs the child leaf itself, so the
        // chain returned by the TA already contains only that leaf (see
        // KeyCreationResult.aidl case 2). The caller (e.g. KeyAttestation) is
        // expected to hold the attestation key's own chain separately — appending
        // it here duplicates the attestation key's certs when the caller fetches
        // both chains, which breaks chain-order verification ("签名错误").
    ...
    fn import_wrapped_key(
        ...
        let wrapping_key_blob = SUPER_KEY
            .read()
            .unwrap()
            .unwrap_key_if_required_with_ommega_compatibility(
...
impl binder::Interface for OmmegaSecurityLevelWrapper {}
...
impl ISecurityLevel for OmmegaSecurityLevelWrapper {
    fn createOperation(
        &self,
        ctx: Option<&CallerInfo>,
        key: &KeyDescriptor,
        ...
        let ctx = Some(require_ommega_ctx(ctx, "ISecurityLevel::createOperation")?);
        let _wp = self.watch("ISecurityLevel::createOperation");
        security_level_manager::notify_operation_performed(self.security_level);
        let (latency, result) =
    ...
    fn generateKey(
        ...
        let ctx = Some(require_ommega_ctx(ctx, "ISecurityLevel::generateKey")?);
        let _wp = self.watch_millis("ISecurityLevel::generateKey", 5000);
        security_level_manager::notify_operation_performed(self.security_level);
        let (latency, result) = crate::timed_call!(self.generate_key(
            ctx,
            key,
    ...
    fn importKey(
        ...
        let ctx = Some(require_ommega_ctx(ctx, "ISecurityLevel::importKey")?);
        let _wp = self.watch("ISecurityLevel::importKey");
        security_level_manager::notify_operation_performed(self.security_level);
        let (latency, result) =
    ...
    fn convertStorageKeyToEphemeral(
        ...
        let ctx = Some(require_ommega_ctx(
            ctx,
            "ISecurityLevel::convertStorageKeyToEphemeral",
        )?);
        let _wp = self.watch("ISecurityLevel::convertStorageKeyToEphemeral");
        security_level_manager::notify_operation_performed(self.security_level);
    ...
    fn deleteKey(&self, ctx: Option<&CallerInfo>, key: &KeyDescriptor) -> Result<(), Status> {
        let ctx = Some(require_ommega_ctx(ctx, "ISecurityLevel::deleteKey")?);
        let _wp = self.watch("ISecurityLevel::deleteKey");
        security_level_manager::notify_operation_performed(self.security_level);
