...
impl crate::KeyMintTa {
    pub(crate) fn begin_operation(
        ...
        let op = match key_material {
            ...
            KeyMaterial::Remote(remote) => Operation {
                handle: op_handle,
                aad_allowed: false,
                input_size: 0,
                slot_to_delete,
                ...
                crypto_op: match purpose {
                    ...
                    _ => {
                        return Err(km_err!(
                            IncompatiblePurpose,
                            "invalid purpose {purpose:?} for remote key",
                        ))
                    }
                },
            },
...
fn remote_sign_algorithm(params: &[KeyParam]) -> Result<String, Error> {
    ...
    match algo {
        ...
        Algorithm::MlDsa => Ok("ML-DSA-65".to_string()),
        _ => Err(km_err!(UnsupportedAlgorithm, "remote sign for {algo:?}")),
    }
}
        ...
        PaddingMode::RsaOaep => {
            ...
            let name = match digest {
                ...
                _ => {
                    return Err(km_err!(
                        UnsupportedDigest,
                        "remote RSA-OAEP decrypt for {digest:?}"
                    ))
                }
            };
