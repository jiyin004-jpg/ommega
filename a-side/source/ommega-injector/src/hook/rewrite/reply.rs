...
pub(super) fn build_ommega_status_reply(status: &Status) -> anyhow::Result<OutboundReply> {
    // ommega is the authoritative keystore backend, so its ServiceSpecific codes
...
fn build_ommega_error_reply(error: &anyhow::Error) -> anyhow::Result<OutboundReply> {
    if let Some(status) = error_status(error) {
        return build_ommega_status_reply(status);
    }

    // A bare StatusCode (no wrapped Status) is an injector-internal failure, not
    // an ommega business error (those arrive as Status, handled above). AOSP
    ...
    // ommega-unavailable codes are already filtered out earlier by
    // ommega_unavailable_error. Errors without any status collapse to SYSTEM_ERROR
    // the same way.
    Ok(synthetic_fallback_reply())
}

pub(super) fn precomputed_ommega_error_reply(error: &anyhow::Error) -> Status {
    if let Some(status) = error_status(error) {
        if status.exception_code() == ExceptionCode::ServiceSpecific {
            return status.clone();
        }
    ...
    // Mirror build_ommega_error_reply: a bare StatusCode (no wrapped Status) is an
...
fn build_ommega_error_reply_or_preserve_system(
    error: &anyhow::Error,
) -> anyhow::Result<Option<OutboundReply>> {
    if ommega_unavailable_error(error) {
        Ok(None)
    } else {
        build_ommega_error_reply(error).map(Some)
    }
}

fn build_ommega_status_reply_or_preserve_system(
    status: &Status,
) -> anyhow::Result<Option<OutboundReply>> {
    if ommega_unavailable_status(status) {
        Ok(None)
    } else {
        build_ommega_status_reply(status).map(Some)
    }
}

fn ommega_error_reply_for_method(
    method: &str,
    caller: &CallerInfo,
    error: &anyhow::Error,
) -> anyhow::Result<Option<OutboundReply>> {
    match build_ommega_error_reply_or_preserve_system(error)? {
        Some(reply) => {
            warn!(
                "event=reply ommega {} failed for uid={} pid={}: {:#}; returning ommega error",
        ...
        None => {
            warn!(
                "event=reply ommega {} unavailable for uid={} pid={}: {:#}; preserving original system reply",
                method, caller.uid, caller.pid, error
            );
            Ok(None)
        }
...
fn ommega_status_reply_for_method(
    method: &str,
    caller: &CallerInfo,
    status: &Status,
) -> anyhow::Result<Option<OutboundReply>> {
    match build_ommega_status_reply_or_preserve_system(status)? {
        Some(reply) => {
            warn!(
                "event=reply ommega {} failed for uid={} pid={}: {:#}; returning ommega error",
...
pub(super) fn build_precomputed_service_reply(
    ...
    match precomputed {
        ...
        PrecomputedServiceReply::GrantSuccess(ommega_grant) => {
            Ok(parcel::build_plain_reply(ommega_grant)?)
        }
        PrecomputedServiceReply::UngrantSuccess | PrecomputedServiceReply::DeleteKeySuccess => {
            Ok(parcel::build_void_reply()?)
        }
        PrecomputedServiceReply::Error(status) => build_ommega_status_reply(status),
    }
}
...
pub(super) fn build_precomputed_maintenance_reply(
    ...
    match precomputed {
        ...
        PrecomputedMaintenanceReply::Error(status) => build_ommega_status_reply(status),
    }
}

pub(super) fn build_no_carrier_ommega_key_entry_reply(
...
fn build_direct_ommega_security_level_reply(
    security_level: crate::android::hardware::security::keymint::SecurityLevel::SecurityLevel,
    pending: &PendingServiceCall,
) -> anyhow::Result<Option<OutboundReply>> {
    match ipc::with_ommega_retry(|ommega| Ok(ommega.r#getSecurityLevel(security_level)?)) {
        Ok(_level) => {
            let carrier = register_synthetic_security_level_carrier(
                security_level,
        ...
        Err(error) => ommega_error_reply_for_method("getSecurityLevel", &pending.caller, &error),
    }
}
...
pub(super) unsafe fn build_service_reply_rewrite(
    ...
    if pending.route != RouteTarget::Ommega {
    ...
    if matches!(
        ...
        if let Err(error) = observe_system_service_reply(tr, pending).map(|_| ()) {
            warn!(
                "event=route failed to retain original System security-level target before ommega reply rewrite: {:#}",
                error
            );
        }
    }
    ...
    match &pending.request {
        ...
        ParsedServiceRequest::UpdateSubcomponent {
            ...
            match ipc::with_ommega_once(|ommega| {
                ...
                Err(error) => {
                    ommega_error_reply_for_method("updateSubcomponent", &pending.caller, &error)
                }
            }
        }
        ParsedServiceRequest::ListEntries { domain, nspace } => {
            match ipc::with_ommega_retry(|ommega| {
                Ok(ommega.r#listEntries(Some(caller), *domain, *nspace)?)
            }) {
                Ok(entries) => Ok(Some(parcel::build_plain_reply(&entries)?)),
                Err(error) => ommega_error_reply_for_method("listEntries", &pending.caller, &error),
            }
        }
        ParsedServiceRequest::Grant {
            ...
            let ommega_grant = match ipc::with_ommega_once(|ommega| {
        ...
        ParsedServiceRequest::GetNumberOfEntries { domain, nspace } => {
            match ipc::with_ommega_retry(|ommega| {
                Ok(ommega.r#getNumberOfEntries(Some(caller), *domain, *nspace)?)
            }) {
                Ok(count) => Ok(Some(parcel::build_plain_reply(&count)?)),
                Err(error) => {
                    ommega_error_reply_for_method("getNumberOfEntries", &pending.caller, &error)
                }
            }
        }
        ParsedServiceRequest::ListEntriesBatched {
            ...
            match ipc::with_ommega_retry(|ommega| {
                Ok(ommega.r#listEntriesBatched(
                    Some(caller),
                    *domain,
                    *nspace,
                ...
                Ok(entries) => Ok(Some(parcel::build_plain_reply(&entries)?)),
                Err(error) => {
                    ommega_error_reply_for_method("listEntriesBatched", &pending.caller, &error)
                }
            }
        }
        ...
        ParsedServiceRequest::DeleteKey { key } => {
            match ipc::with_ommega_once(|ommega| Ok(ommega.r#deleteKey(Some(caller), key)?)) {
                Ok(()) => Ok(Some(parcel::build_void_reply()?)),
                Err(error) => ommega_error_reply_for_method("deleteKey", &pending.caller, &error),
            }
        }
    }
}
...
pub(super) unsafe fn build_security_level_reply_rewrite(
    ...
    build_ommega_security_level_reply(pending, (tr.flags & super::super::binder::TF_ONE_WAY) == 0)
}

pub(super) fn build_ommega_security_level_reply(
    pending: &PendingSecurityLevelCall,
    publish_operation_carrier: bool,
) -> anyhow::Result<Option<OutboundReply>> {
    if let Err(error) = ensure_mirror_state_recovered() {
        warn!(
            "event=route ommega security-level {:?} blocked by unresolved mirror recovery for uid={} pid={}: {:#}",
            pending.request.method(), pending.caller.uid, pending.caller.pid, error
        );
        return Ok(Some(synthetic_fallback_reply()));
    }
    ...
    match &pending.request {
        ...
        ParsedSecurityLevelRequest::GenerateKey {
            ...
            match ommega_level.r#generateKey(
                Some(caller),
                key,
        ...
        ParsedSecurityLevelRequest::ImportKey {
            ...
            match ommega_level.r#importKey(
                Some(caller),
                key,
                attestation_key.as_ref(),
                params,
                *flags,
                ...
                Ok(metadata) => Ok(Some(parcel::build_plain_reply(&metadata)?)),
                Err(error) => ommega_status_reply_for_method("importKey", &pending.caller, &error),
            }
        }
        ParsedSecurityLevelRequest::ImportWrappedKey {
            ...
            match ommega_level.r#importWrappedKey(
                Some(caller),
                key,
                wrapping_key,
                ...
                Ok(metadata) => Ok(Some(parcel::build_plain_reply(&metadata)?)),
                Err(error) => {
                    ommega_status_reply_for_method("importWrappedKey", &pending.caller, &error)
                }
            }
        }
        ...
        ParsedSecurityLevelRequest::DeleteKey { key } => {
            match ommega_level.r#deleteKey(Some(caller), key) {
                Ok(()) => Ok(Some(parcel::build_void_reply()?)),
                Err(error) => ommega_status_reply_for_method("deleteKey", &pending.caller, &error),
            }
        }
    }
}
